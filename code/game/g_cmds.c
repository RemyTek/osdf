...
void DeathmatchScoreboardMessage(gentity_t* ent) {
	...
	for (i = 0; i < numSorted; i++) {
		...
		j = BG_sprintf(
			entry, " %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.sortedClients[i], cl->ps.persistant[PERS_SCORE], ping,
...
static void G_SayTo(gentity_t* ent, gentity_t* other, int mode, int color, const char* name, const char* message) {
	...
	trap_SendServerCommand(
		other - g_entities, va("%s \"%s%c%c%s\" %i", mode == SAY_TEAM ? "tchat" : "chat", name, Q_COLOR_ESCAPE, color, message, ent - g_entities));
}
...
static const char* voteCommands[] = {"map_restart", "map",        "rotate",   "nextmap",   "kick",      "clientkick",
                                     "g_gametype",  "g_unlagged", "g_warmup", "timelimit", "fraglimit", "capturelimit"};
...
static qboolean ValidVoteCommand(int clientNum, char* command) {
	...
	*s = '\0';
	...
	if (i == ARRAY_LEN(voteCommands)) {
		trap_SendServerCommand(
			clientNum, "print \"Invalid vote command.\nVote commands are: \n"
