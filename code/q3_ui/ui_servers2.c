...
static void ArenaServers_UpdateList(void) {
	...
	for (i = 0, j = 0; i < count; i++, servernodeptr++) {
		...
		Com_sprintf(
			tableptr->buff, sizeof(tableptr->buff), "%-*.*s %-*.*s %2d/%2d %-*.*s %3s %s%3d", MAX_HOSTNAMELENGTH, MAX_HOSTNAMELENGTH, servernodeptr->hostname,
			MAX_MAPNAMELENGTH, MAX_MAPNAMELENGTH, servernodeptr->mapname, servernodeptr->numclients, servernodeptr->maxclients, MAX_GAMENAMELENGTH,
			MAX_GAMENAMELENGTH, servernodeptr->gamename, netnames[servernodeptr->nettype], pingColor, servernodeptr->pingtime);
		j++;
	}
...
static void ArenaServers_UpdateMenu(void) {
	...
	if (g_arenaservers.numqueriedservers > 0) {
		...
		if (g_arenaservers.refreshservers && (g_arenaservers.currentping <= g_arenaservers.numqueriedservers)) {
			// show progress
			Com_sprintf(
				g_arenaservers.status.string, MAX_STATUSLENGTH, "%d of %d Arena Servers.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
...
static void ArenaServers_Remove(void) {
	...
	if (i <= g_arenaservers.numfavoriteaddresses - 1) {
		if (i < g_arenaservers.numfavoriteaddresses - 1) {
			// shift items up
			memcpy(
				&g_arenaservers.favoriteaddresses[i], &g_arenaservers.favoriteaddresses[i + 1],
...
static void ArenaServers_MenuInit(void) {
	...
	g_arenaservers.filter.generic.right =
		g_arenaservers.filter.generic.x + (g_arenaservers.filter.field.widthInChars + FILTER_CAPTION_CHARS) * SMALLCHAR_WIDTH + 1;
