...
static void CG_DrawStatusBar(void) {
  ...
  //		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	CG_DrawTeamBackground(
		cgs.screenXmin, cgs.screenYmax - STATUSBAR_HEIGHT + 1, cgs.screenXmax - cgs.screenXmin + 1, STATUSBAR_HEIGHT, 0.33f, cg.snap->ps.persistant[PERS_TEAM]);
	...
	if (cent->currentState.weapon && cg_weapons[cent->currentState.weapon].ammoModel) {
		...
		CG_Draw3DModel(CHAR_WIDTH * 3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE, cg_weapons[cent->currentState.weapon].ammoModel, 0, origin, angles);
	}
	...
	if (ps->stats[STAT_ARMOR]) {
		...
		CG_Draw3DModel(370 + CHAR_WIDTH * 3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE, cgs.media.armorModel, 0, origin, angles);
	}
#ifdef MISSIONPACK
...
static int CG_DrawPickupItem(int y) {
	...
	if (value) {
		...
		if (fadeColor) {
			...
			CG_DrawString(
				cgs.screenXmin + PICKUP_ICON_SIZE + 16, y + (PICKUP_ICON_SIZE / 2 - PICKUP_TEXT_SIZE / 2), text, fadeColor, PICKUP_TEXT_SIZE, PICKUP_TEXT_SIZE,
				0, DS_SHADOW | DS_PROPORTIONAL);
...
#define CHATLOC_X (cgs.screenXmin)
	...
	if (cgs.teamLastChatPos != cgs.teamChatPos) {
		...
		for (i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i--) {
			CG_DrawString(
				CHATLOC_X + TINYCHAR_WIDTH, CHATLOC_Y - (cgs.teamChatPos - i) * TINYCHAR_HEIGHT, cgs.teamChatMsgs[i % chatHeight], hcolor, TINYCHAR_WIDTH,
				TINYCHAR_HEIGHT, 0, DS_PROPORTIONAL);
		}
	}
}
#endif  // MISSIONPACK
...
static void CG_DrawCrosshair(void) {
	...
	trap_R_DrawStretchPic(
		x + cg.refdef.x + 0.5 * (cg.refdef.width - w) - cgs.screenXBias, y + cg.refdef.y + 0.5 * (cg.refdef.height - h) - cgs.screenYBias, w, h, 0, 0, 1, 1,
		hShader);
}
...
static void CG_DrawSpectator(void) {
	...
	if (cgs.gametype == GT_TOURNAMENT) {
		...
		CG_DrawString(
			320, cgs.screenYmax - 20 + 1, "press ESC and use the JOIN menu to play", colorWhite, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, 0,
			DS_SHADOW | DS_CENTER | DS_PROPORTIONAL);
	}
}
...
static void CG_DrawProxWarning(void) {
	...
	CG_DrawString(
		320, 64 + 64 + BIGCHAR_HEIGHT, s, g_color_table[ColorIndex(COLOR_RED)], BIGCHAR_WIDTH, BIGCHAR_HEIGHT, 0, DF_SHADOW | DF_FORCE_COLOR | DF_CENTER);
#endif
